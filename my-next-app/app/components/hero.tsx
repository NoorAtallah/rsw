  ...
  useEffect(() => {
    async function fetchVideo() {
      ...
      const { data } = await supabase
        ...
        .eq('key', 'video')
        .single()
      if (data?.value_en) setVideoUrl(data.value_en)
    }
    fetchVideo()
  }, [])
    ...
    <div className="min-h-screen h-screen relative w-full" style={{ background: black, overflow: 'hidden' }}>
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; }
        ...
        .text-vertical-lr { writing-mode: vertical-lr; text-orientation: mixed; }
        .text-vertical-ar { writing-mode: vertical-rl; text-orientation: mixed; transform: rotate(180deg); }
        .hero-video-overlay { background: linear-gradient(135deg, rgba(0,0,0,0.4) 0%, rgba(0,0,0,0.3) 50%, rgba(0,0,0,0.2) 100%); }
        .hero-video-overlay-vignette { background: radial-gradient(circle at center, transparent 0%, rgba(0,0,0,0.3) 100%); }
        .stat-glow { text-shadow: 0 0 40px rgba(167,147,112,0.5), 0 0 80px rgba(167,147,112,0.3); }
        .glass-border-gold { border: 1px solid rgba(167,147,112,0.4); }
        .glass-bg-gold { background: rgba(167,147,112,0.1); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); }
      `}</style>
      ...
      <div
        ...
        style={{ background: `rgba(167, 147, 112, 0.2)`, [isRTL ? 'left' : 'right']: '0' }}
      />
      <div
        ...
        style={{ background: `rgba(167, 147, 112, 0.15)`, [isRTL ? 'right' : 'left']: '0' }}
      />
      ...
      <div className="relative h-full z-10 w-full overflow-hidden" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <motion.section
          ...
          style={{ top: '22%', insetInlineStart: '0', insetInlineEnd: '0', maxWidth: '100%' }}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4">
              ...
              <Link href="/investor-relations">
                <button
                  ...
                  onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(167, 147, 112, 0.1)'; e.currentTarget.style.color = white }}
                >
        ...
        >
          <span
            ...
            style={{ color: gold, fontFamily: monoFont, fontSize: '0.75rem', letterSpacing: locale === 'ar' ? 'normal' : '0.5em', textTransform: locale === 'ar' ? 'none' : 'uppercase', opacity: 0.6 }}
          >
            {t('hero.verticalText')}
          </span>
        </motion.div>
        ...
        >
          <span className="text-[280px] font-bold tracking-wider leading-none" style={{ color: gold, fontFamily }}>RSW</span>
        </motion.div>
        ...
        <motion.div
          ...
          style={{ top: '24%', [isRTL ? 'right' : 'left']: '20%', background: isRTL ? `linear-gradient(to left, ${gold}, transparent)` : `linear-gradient(to right, ${gold}, transparent)` }}
          initial={{ scaleX: 0 }}
        ...
        <motion.div
          ...
          style={{ top: '24%', [isRTL ? 'right' : 'left']: '20%', background: `linear-gradient(to bottom, ${gold}, transparent)` }}
          initial={{ scaleY: 0 }}
        ...
        >
          <Link href="/contact">
            <div
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(167, 147, 112, 0.2)' }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(167, 147, 112, 0.1)' }}
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: gold }}>
                  <ArrowUpRight className="w-5 h-5" strokeWidth={2} style={{ color: black, transform: isRTL ? 'scaleX(-1)' : 'none' }} />
                </div>
                <div>
                  <p className="text-sm font-semibold" style={{ color: white, fontFamily }}>{t('hero.ctaCard.title')}</p>
                  <p className="text-xs" style={{ color: gold, fontFamily: monoFont }}>{t('hero.ctaCard.subtitle')}</p>
                </div>
              </div>
            </div>
          </Link>
